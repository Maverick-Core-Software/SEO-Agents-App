...
def build_facebook_crew(
    ...
    fb_context = (
        f"START DATE: {start_date or 'Next business day'}\n"
        f"DAYS TO SCHEDULE: {days}\n\n"
        "VIDEO DAYS: Days 1, 4, and 7 must be VIDEO posts. Day 5 must be a TEXT-only post (no photo) — these have tested well organically. "
        "All other days (2, 3, 6) are PHOTO posts (or TEXT if no photo fits).\n\n"
    ...
    fb_task = Task(
        description=(
            ...
            "- TYPE must be: video\n"
            "- VIDEO_PROMPT is a scene idea for an attention-grabbing vertical Reel (a director step rewrites it "
            "before generation). Make it DRAMATIC and fast-paced: open on the electrical problem at its scariest "
            "(sparks, arc flash, blackout, storm), 3-4 quick cuts, human stakes, end with the electrician arriving "
            "or power triumphantly restored\n"
            "- Example: 'An outlet erupts in sparks as a family watches TV — the whole house plunges dark. Quick "
            "cuts: worried faces lit by phone flashlight, a breaker panel crackling, an electrician striding in "
            "with a tool bag. Lights surge back on room by room. Stormy night, cinematic, fast cuts, 8 seconds'\n"
            ...
            "TEXT POST RULES (day 5):\n"
            "- TYPE must be: text (no photo)\n"
            "- PHOTO_FILE: leave blank\n\n"
