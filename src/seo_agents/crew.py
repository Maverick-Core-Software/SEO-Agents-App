...
def build_facebook_crew(
    ...
    Media policy: Day 1 slideshow Reel from real photos; Days 3/5 photo or
    carousel; Day 6 photo or text. No AI-generated video.
    ...
    fb_context = (
        ...
        "MEDIA POLICY (mandatory — real photos only, NO AI-generated video):\n"
        "  Day 1 (Mon) = SLIDESHOW (Ken Burns Reel built from real job photos + on-screen text).\n"
        "  Day 3 (Wed) = PHOTO or CAROUSEL (carousel when 2+ related job photos fit the topic).\n"
        "  Day 5 (Fri) = PHOTO or CAROUSEL (prefer before/after multi-photo when available).\n"
        "  Day 6 (Sat) = PHOTO or TEXT.\n"
        "Never set TYPE to video. Do not invent AI video scenes. "
        "Days 2, 4, 7: NO POSTS (these are skipped by the poster). "
        "Posting 7x/week with ~200 followers signals 'low quality page' to Facebook's algorithm — "
        "4 posts per week is the sweet spot for this follower count.\n\n"
        f"SEASONAL CONTEXT: {seasonal_hint}\n\n"
        "AVAILABLE PHOTOS (use these for PHOTO_FILE / slideshow topics):\n"
        f"{photo_list}\n\n"
        "CONTENT REPORT:\n\n"
        f"{content_report}\n\n"
    ...
    fb_agent = Agent(
        ...
        goal=(
            ...
            "Media is real job photos and photo slideshows only — never AI-generated video."
        ),
        backstory=(
    ...
    fb_task = Task(
        description=(
            ...
            "- Contact info goes in a separate CONTACT field (add to format below), "
            "NOT in the caption CTA. The poster script posts fixed contact copy (the "
            "text-us quote line) as the first comment.\n"
            ...
            "  1. Before/After transformation (real photos): Show problem then solution\n"
            "  2. Educational/How-To: Teach something useful (signs of failing panel, GFCI basics)\n"
            "  3. Behind-the-Scenes/Day-in-the-Life: Real job photos, van, materials, completed work\n"
            ...
            "SLIDESHOW POST RULES (Day 1 only — one Reel per week from REAL photos):\n"
            "- TYPE must be: slideshow\n"
            "- The poster builds a vertical 9:16 Ken Burns slideshow from curated job photos "
            "(no AI video generation).\n"
            "- ON_SCREEN_TEXT is required — most people watch without sound. Use 3-5 short beats:\n"
            "  * [0:00–0:03] hook line\n"
            "  * [0:04–0:08] problem / what homeowners miss\n"
            "  * [0:09–0:14] what to do / what Grizzly installs\n"
            "  * [0:15–0:18] local payoff (DFW / Rowlett)\n"
            "- PHOTO_FILE: pick the best real photo filename for the topic from AVAILABLE PHOTOS "
            "(the poster may add more same-date/service curated stills).\n"
            "- VIDEO_PROMPT: leave blank (do not write AI scene prompts).\n"
            "- NEVER invent fake job photos or describe synthetic video scenes.\n\n"
            "PHOTO / CAROUSEL / TEXT POST RULES (Days 3, 5, and 6):\n"
            "- Day 3 or 5: TYPE must be photo OR carousel. Use carousel when the topic has 2+ "
            "real job photos (before/after, multi-step, panel open + finished). "
            "Otherwise use photo.\n"
            "- Day 6: TYPE must be photo (if a photo fits) or text.\n"
            "- PHOTO_FILE: pick the photo whose filename best matches the post's SERVICE/topic. "
            "For carousel you may list 2-6 filenames separated by commas; the poster also pulls "
            "same-date/service curated stills. A later deterministic step refines PHOTO_FILE.\n"
            "- VIDEO_PROMPT and ON_SCREEN_TEXT: leave blank on photo/carousel/text days.\n\n"
            ...
            "TYPE: [slideshow|photo|carousel|text|skip]\n"
            ...
            "CONTACT: [always this exact line: 📲 Text us at (469) 896-3862 to get a free "
            "instant quote — calls welcome too! — posted as first comment, not in caption]\n"
            "PHOTO_FILE: [real photo path/filename or blank]\n"
            "VIDEO_PROMPT: [always blank — AI video disabled]\n"
            "ON_SCREEN_TEXT: [slideshow text beats for Day 1, else blank]\n"
            ...
            "   THIS SECTION IS EXECUTED BY A MACHINE, NOT A HUMAN. A deterministic\n"
            "   script reads it and spends real money from it. It must be a single,\n"
            "   final, unambiguous allocation — there is nobody downstream to make a\n"
            "   judgement call, and any ambiguity means the whole week's boost is\n"
            "   skipped and the budget is wasted.\n\n"
            "   HARD RULES — violating any one of these voids the entire week's boost:\n"
            "   - The YES rows must sum to EXACTLY $50. Not 'up to $50', not $50 each.\n"
            "     Sum every YES row's (daily x days) and check the arithmetic before\n"
            "     you write the total. If two posts are each marked YES at $25/day x\n"
            "     2 days, that is $100 and it is WRONG.\n"
            "   - Decide NOW which post gets the money. Never defer the choice with\n"
            "     'whichever performs better', '— OR —', 'if Day 1 underperforms',\n"
            "     'shift the $50 to', 'hold the boost', 'only one boost runs', or any\n"
            "     other conditional or either/or wording. Those exact phrases cause a\n"
            "     hard abort. Pick one allocation and commit to it.\n"
            "   - MAYBE means $0. A MAYBE row must have '—' for daily budget,\n"
            "     duration, and total. Never attach dollars to a MAYBE.\n"
            "   - NO means $0, same formatting.\n"
            "   - You may split the $50 across at most two posts (e.g. Day 1 $25/day x\n"
            "     1 day = $25 and Day 3 $25/day x 1 day = $25). If you split it, BOTH\n"
            "     are YES and both actually run — a split is not a contingency.\n"
            "   - Each post's per-post BOOST/BOOST_AMOUNT/BOOST_DURATION fields must\n"
            "     match its row here exactly. This table wins on conflict, but do not\n"
            "     create a conflict.\n\n"
            "   Format the section as a markdown table with these columns:\n"
            "   | Post | Day | Service | Boost Decision | Daily Budget | Duration | Total |\n"
            "   with one row per post, the Post cell starting with 'Day N', and the\n"
            "   Boost Decision cell containing exactly one of YES, MAYBE, or NO.\n\n"
            "   Then add, as plain lines after the table:\n"
